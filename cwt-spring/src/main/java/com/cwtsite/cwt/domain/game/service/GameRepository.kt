...
interface GameRepository : JpaRepository<Game, Long> {
    ...
    @Query(
        "select g from Game g where (g.homeUser = :user or g.awayUser = :user) and (g.homeUser is not null " +
    ...
    @Query(
        "select g from Game g where g.tournament = :tournament and g.playoff.round = :round " +
    ...
    @Query(
        value = "select g from Game g where g.playoff.round >= :finalRound and g.homeUser is not null and g.awayUser is not null " +
    ...
    fun findByTournamentAndRoundAndNotVoided(
        @Param("tournament") tournament: Tournament,
        @Param("round") round: Int
    ): List<Game>
