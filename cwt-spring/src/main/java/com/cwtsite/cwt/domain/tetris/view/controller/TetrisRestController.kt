...
class TetrisRestController {
    ...
    fun saveTetris(@RequestBody tetrisDto: TetrisDto): ResponseEntity<TetrisDto> {
        if (tetrisDto.guestname != null && !userService.validateUsername(tetrisDto.guestname)) {
            throw RestException("Invalid Username.", HttpStatus.BAD_REQUEST, null)
        }
        return ResponseEntity.ok(
                tetrisMapper.toDto(tetrisService.add(null, tetrisDto.highscore, tetrisDto.guestname)))
    }
