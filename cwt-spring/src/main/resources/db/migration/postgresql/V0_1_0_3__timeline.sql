...
from (select TO_CHAR(t.CREATED, 'yyyy')                                                                                      as year,
             least(greatest((case coalesce(g.g_id, 0) when 0 then 0 else 1 end), max(po.round) + 1), t.max_rounds - 1) + (case
      ...
      group by year, g.g_id, t.GOLD_WINNER_ID, t.SILVER_WINNER_ID, t.BRONZE_WINNER_ID, t.max_rounds
